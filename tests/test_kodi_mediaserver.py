...
class TestKodiClient:
    ...
    def test_get_libraries(self):
        ...
        assert len(result) == 2
        ids = {lib['id'] for lib in result}
        assert ids == {'movies', 'tvshows'}
        types = {lib['id']: lib['type'] for lib in result}
    ...
    def test_get_items_movies(self, mock_connect):
        ...
        mock_connect.return_value = mock_conn
        items = list(kodi.get_items(parent='movies'))
    ...
    def test_get_items_tvshows(self, mock_connect):
        ...
        mock_connect.return_value = mock_conn
        items = list(kodi.get_items(parent='tvshows'))
        items = [i for i in items if i]
        assert len(items) == 1
    ...
    def test_get_items_no_duplicate_with_multiple_uniqueid(self, mock_connect):
        ...
        assert item_ids == ['movie:1', 'movie:2', 'movie:3']
    ...
    def test_get_items_group_by_full_columns(self, mock_connect):
        ...
        list(kodi.get_items(parent='movies'))
        ...
        # 单独验证 tvshows 的 GROUP BY
        mock_conn2 = TrackingConnection([[]])
        mock_connect.return_value = mock_conn2
        list(kodi.get_items(parent='tvshows'))
        cursor2 = mock_conn2.last_cursor
        assert len(cursor2.executed) == 1
        tvshow_sql = cursor2.executed[0][0]
